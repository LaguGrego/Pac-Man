...
Fantasma_2::Fantasma_2(int X,int Y) {
	llenar_animacion_fantasma("AnimacionFantasma/Fantasma_Azul.png");
	SF.setOrigin(8.f,8.f);
	SF.setPosition(X,Y);
}

///Fantasma que huye de PACMAN (Fantasma cagón)

void Fantasma_2::mover(sf::Vector2f posicion_pacman,char Mapa[30][41]){
	...
	if(exactx and exacty and presa==true){
		...
		//crea un espacio 200x200 alrededor de pacman que el fantasma persigue, una vez dentro de este espacio el fantasma se comporta aleatorio.
		else if (Mapa[M_y][M_x]=='|' || Mapa[M_y][M_x]=='_'){
			if	   (posicion_pacman.x<=SF.getPosition().x and posicion_pacman.y>=SF.getPosition().y and not(posicion_pacman.x+100>SF.getPosition().x and posicion_pacman.y-100<SF.getPosition().y)){
				int X=rand()%2;
			...
			else if	   (posicion_pacman.x>=SF.getPosition().x and posicion_pacman.y>=SF.getPosition().y and not(posicion_pacman.x-100<SF.getPosition().x and posicion_pacman.y-100<SF.getPosition().y)){
				int X=rand()%2;
